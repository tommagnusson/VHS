...
public class Shuffler {
	...
	public static void perfectShuffle(int[] values) {
		...
		int k = 0;
		//   0 to half of the ordered deck
		//   fill every other index of shuffled deck
		//   with  the first half of the ordered deck
		...
		k = 1;
		//   half of deck indicies to the last index
		//   fill every other odd index of shuffled
		//   with the second half of the ordered deck
	...
	public static void selectionShuffle(int[] values) {
		for (int k = values.length - 1; k >= 0; k--) {
			int random = (int) ((Math.random() * k) + 1); // [0, k]
			int temp = values[k]; // to swap kth card with randomth card, where random is [0, k]
